...
This notebook is a supplementary material of the paper "On the use of local $\hat{R}$ to improve MCMC convergence diagnostic".
...
\end{equation}
In addition to the $\hat{R}(x)$ and $R(x)$ curves, we suggest to use $R_\infty = \sup_{x \in\mathbb{R}} R(x)$ as a scalar summary.
...
The code that is used is also available online (https://github.com/TheoMoins/localrhat).
...
For $m=2$, $1.01$ is an adapted threshold:
...
We consider two cases where $\hat{R}$ is not able to detect a convergence failure,
...
WARNING: in the case where $d=2$, the orders on the two bounds PLOD and NLOD are 
inverted (compared to the asymptotic behavior), and that is why here we are 
more sensitive when $\rho < 0$.
